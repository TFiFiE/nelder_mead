...
template<class Number,unsigned int dimensions>
struct NelderMead {
  ...
  NelderMead(const Container& container) :
    NelderMead(begin(container),end(container))
  {
  }

  static Input& input(Vertex& vertex)
  {
    return vertex.first;
  }

  static const Input& input(const Vertex& vertex)
  {
    return vertex.first;
  }

  static Number& output(Vertex& vertex)
  {
    return vertex.second;
  }

  static const Number& output(const Vertex& vertex)
  {
    return vertex.second;
  }

  static bool isBetterThan(const Vertex& lhs,const Vertex& rhs)
  {
    return output(lhs)<output(rhs);
  }

  template<class Iterator>
  static Input centroid(const Iterator begin,const Iterator end,const Number oneOverDistance)
  {
    Input result={{0}};
  ...
  static Vertex extrapolate(const Input& origin,const Vertex& reference,const Number factor,Function function,const Step step)
  {
    Vertex result;
    for (unsigned int c=0;c<dimensions;++c)
  ...
  static NelderMead<Number,dimensions> create(const Iterator begin,const Iterator end,Function function)
  {
  ...
  static NelderMead<Number,dimensions> create(const Container& container,Function function)
  {
    return create(begin(container),end(container),function);
  }

  template<class Generator>
  static std::vector<Input> randomPolytope(Generator& generator,const std::array<std::pair<Number,Number>,dimensions>& ranges,const unsigned int numVertices=dimensions+1)
  {
